...
package Lkql_Checker.Ids is
   ...
   is (Find (All_Rule_Ids, To_Lower (To_Text (Rule_Name)), True));
   ...
   is (Image (To_Symbol (All_Rule_Ids, Id)));
   ...
   package Exemption_Id_Vec is new
     Ada.Containers.Indefinite_Vectors
       (Index_Type   => Natural,
        Element_Type => Exemption_Id);
   ...
   function Find_Exemption_Id (Exempted_Name : String) return Exemption_Id
   is (Find (All_Exemption_Ids, To_Lower (To_Text (Exempted_Name)), True));
   --  Get an exemption id for the given ``Exempted_Name``. The given name is
   --  normalized before being put in the symbol table.

   function Hash (Id : Exemption_Id) return Ada.Containers.Hash_Type
