...
--  This package defines "rules" for getting the information for the final
...
package Lkql_Checker.Compiler is
   ...
   --  Returns whether the current checker process should set the "codepeer"
   ...
   -----------------------------------------------
   -- Using in checks performed by the compiler --
   -----------------------------------------------
   ...
   --  these checks as a part of checker run and to incorporate the results
   --  of these checks into the checker report.
   ...
   --  The checker uses three parametric rules to activate these checks:
   ...
   --  Restriction_Warnings pragmas generated by the checker should be used.

   procedure Set_Compiler_Checks;
   --  Sets the values of Use_gnaty_Option, Use_gnatw_Option and
   --  Check_Restrictions on the base of setting made by the warnings,
   --  style-checks and restrictions rules options.

   Analyze_Compiler_Output : Boolean := False;
   --  Boolean flag indicating if the checker should spawn the compiler and
   --  analyze its output.
   ...
   --  given checker call, without adding anything to it and with no leading
   --  -gnaty.
   ...
   --  checker call, without adding anything to it and with no leading
   --  -gnatw.
   ...
   --  for the given checker call (with the corresponding parameter value, if
